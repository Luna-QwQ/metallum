...
public class MixinModelStorageTrigger {
	...
	private <S> void iris$changeRenderType2(PoseStack poseStack, RenderType renderType, int outlineColor, SubmitNodeCollector.CustomGeometryRenderer customGeometryRenderer, Operation<Void> original) {
		if (ImmediateState.isRenderingBEs) {
			renderType = OuterWrappedRenderType.wrapExactlyOnce("iris:block_entity", renderType, BlockEntityRenderStateShard.INSTANCE);
		}

		original.call(poseStack, renderType, outlineColor, customGeometryRenderer);
	}
